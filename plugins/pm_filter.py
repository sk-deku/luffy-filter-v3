...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if st.status == enums.ChatMemberStatus.OWNER or str(userid) in ADMINS:
            ...
            await query.answer("You need to be Group Owner or an Auth User!", show_alert=True)
        ...
        stat, cb = ("CONNECT", "connectcb") if not act else ("DISCONNECT", "disconnect")
        ...
        hr = await client.get_chat(int(group_id))
        title = hr.title
        if await make_active(str(userid), str(group_id)):
        ...
        hr = await client.get_chat(int(group_id))
        title = hr.title
        if await make_inactive(str(userid)):
        ...
        try:
            ...
            if user and user.get('tokens', 0) > 0:
                await db.update_user(userid, {'$inc': {'tokens': -1}})
                ...
                await query.answer('You do not have enough tokens to download this file.', show_alert=True)

        except UserIsBlocked:
            await query.answer('Sorry, you are blocked by an Admin!', show_alert=True)
        ...
        await query.message.edit_text(script.START_TXT.format(query.from_user.mention, temp.U_NAME, temp.B_NAME),
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(script.HELP_TXT.format(query.from_user.mention),
                                      reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(script.ABOUT_TXT.format(temp.B_NAME),
                                      reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)
