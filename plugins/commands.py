...
async def start(client, message):
    ...
    # ✅ Ensure `user_id` is defined before using it
    if message.from_user:
        user_id = message.from_user.id
    else:
        return  # Exit if `from_user` is None

    data = message.command[1] if len(message.command) == 2 else None

    # ✅ Safely check if user clicked a verification link
    if user_id in active_tokens and active_tokens.get(user_id, {}).get("token") == data:
        if time.time() > active_tokens[user_id].get("expires", 0):  # Default to 0 if key is missing
...
async def verify(client, message):
    ...
    active_tokens[user_id] = {"token": hashed_token, "expires": time.time() + TOKEN_EXPIRY_TIME}

    # Generate the shortened link for the token
    redirect_url = f"https://t.me/{BOT_USERNAME}?start={hashed_token}"
    short_link_request = requests.get(SHORTENER_URL.format(api_key=SHORTENER_API_KEY, url=redirect_url))
    short_link = short_link_request.json().get("shortenedUrl", redirect_url)

    # Send the verify message with buttons
    buttons = [
...
async def check_tokens(client, message):
    ...
    await message.reply(f"💰 Your current token balance: {token_balance} tokens")
